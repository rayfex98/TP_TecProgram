...
EXEC ALERTAPROC @ID = INT,@STOCK = INT,@USUARIO = INT,@MINIMO = INT,@TIPO = NVARCHAR(10);
*/ 
...
EXEC CATEGORIAPROC @ID = INT,@DESCRIPCION = VARCHAR(30),@TIPO = NVARCHAR(10);
*/
...
EXEC DIRECCIONPROC @ID=INT,@ALTURA=VARCHAR(6),@CALLE=VARCHAR(30),@CP=VARCHAR(15),@LOCALIDAD=VARCHAR(20),@PROVINCIA=VARCHAR(20),@TIPO=NVARCHAR(10);
*/
	...
	BEGIN
		...
		IF @TIPO = 'DELETE'
			BEGIN
				...
				WHERE ([IDDETALLE] = @ID OR [IDORDEN] = @ORDEN)
			END
	END
GO
...
EXEC ORDENCOMPRAPROC @ID=INT,@PROVEEDOR=INT,@USUARIO=INT,@FECHA=SMALLDATETIME,@TIPO=NVARCHAR(10);
*/
	...
	BEGIN
		...
		IF @TIPO = 'SELECTID'
			BEGIN
				SELECT	[IDORDEN]
				FROM [dbo].[ORDENCOMPRA]
				WHERE [IDPROVEEDOR] = @PROVEEDOR AND [IDPERSONA] = @USUARIO
			END
		IF @TIPO = 'UPDATE'
			BEGIN
...
EXEC PROVEEDORPROC @ID=INT,@DIRECCION=INT,@CUIL=VARCHAR(13),@RAZONSOCIAL=VARCHAR(30),@TIPO=NVARCHAR(10);
*/
...
EXEC STOCKPROC @ID=INT,@PRODUCTO=INT,@CANTIDAD=INT,@TIPO=NVARCHAR(10);
*/
...
EXEC USUARIOPROC @ID=INT,@ROL=TINYINT,@LEGAJO=INT,@TIPO=NVARCHAR(10);
*/
